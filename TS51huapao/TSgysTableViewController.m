...
#import "TSgysTableViewController.h"
#import "AFNetworking.h"
#import "TSmyzhixiaoTableViewController.h"
#import "TSMyInfoTableViewController.h"
#import "TSMyComInfoTableViewController.h"

static NSString*const BaseURLString = @"http://124.232.163.242/com.ds.ws/FOXHttpHandler/FoxGetAnVipTradeCondition.ashx?U_type=S&ConditionType=TO&CardCode=s0008";

@interface TSgysTableViewController ()
...
@implementation TSgysTableViewController
...
- (void)viewDidLoad
{
    ...
    _itemArray = [@[@"企业资料",@"个人资料",@"我的产品库",@"我的订单",@"发货单",@"产品关注",@"我的消息",@"会员退出"]mutableCopy];
    [self _initData];
...
- (void)_initsectionView
{
    ...
    nameLabel.text = @"湖南省浏阳市世纪红烟花制造工厂（李友钦）";
    [nameLabel setBackgroundColor:[UIColor whiteColor]];
    
    for (int i = 0; i < 2; i ++) {
        UIView * view = [[UIView alloc] initWithFrame:CGRectMake(10+i*( (ScreenWidth-25)/2 +5), 75, (ScreenWidth-30)/2, 50)];
        view.tag = 100 + i;
    ...
    for (int i = 0; i<2; i++) {
        ...
        if (self.getDic != nil) {
            if (i == 0) {
                ...
                xiangxilabel.text = [NSString stringWithFormat:@"%@",[self.getDic objectForKey:@"OnHand"]];
            }
